...
function main()
	...
	sampRegisterChatCommand("tgsend", function(test)
		...
		if test ~= nil and #test > 0 then
				if test:find("%d+ .*") then
					local HouseNum, HouseArea = test:match("(%d+) (.*)")
					if HouseNum and HouseArea then
						local ip = sampGetCurrentServerAddress()
						if ip == "185.169.134.83" then
							prefixTG = "[RPG]"
					local send = ""..prefixTG.." Комментарий от "..sampGetPlayerNickname(id)..": Слетел дом "..HouseNum.." в районе: "..HouseArea..""
				...
				sampAddChatMessage("[GC]:{8be547} Правильное использование: /tgsend ДОМ РАЙОН.", -1)
			end
		else
			sampAddChatMessage("[GC]:{8be547} Правильное использование: /tgsend ДОМ РАЙОН.", -1)
		end
